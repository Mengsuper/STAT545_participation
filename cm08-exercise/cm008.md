...
![](cm008_files/figure-markdown_github/unnamed-chunk-10-1.png)
...
gapminder %>% 
  group_by(country) %>% 
  mutate(change = pop - lag(pop)) # compare to the previous year
```
...
```
    ...
    ## Warning in maxsum:length(ll): numerical expression has 1704 elements: only
    ## the first used
    ...
    ##  Zimbabwe: 0   Oceania: 0   Min.   :1952   Min.   :68.75  
    ##  (Other) :12   (Other):12   1st Qu.:1966   1st Qu.:71.92  
    ##                             Median :1980   Median :74.98  
    ##                             Mean   :1980   Mean   :74.90  
    ##                             3rd Qu.:1993   3rd Qu.:78.11  
    ##                             Max.   :2007   Max.   :80.65  
    ##       pop             gdpPercap        cda_gpd         
...
``` r
gapminder %>% 
  mutate(pop_in_billions = pop / 1000000)
```

    ## # A tibble: 1,704 x 7
    ##    country     continent  year lifeExp      pop gdpPercap pop_in_billions
...
2.  Rank the continents by GDP per capita. You should have two columns: one with continent, and another with the ranking (1 through 5). **Hint**: use the `rank()` or `min_rank()` function.
